...
use Wit\ModelBundle\Entity\Gender;
use Wit\SettingBundle\Form\GenderType;
...
class GenderController extends Controller
{
    ...
    public function indexAction($id=null)
    {
        ...
        $entities = $em->getRepository('ModelBundle:Gender')->findBy(array(), array('id' => 'DESC'));

        //form to create
        $addform = $this->createCreateForm(new Gender());

        //if user wants to edit a record
        if($id){
            $editEntity = $em->getRepository('ModelBundle:Gender')->find($id);
            if (!$editEntity) {
    ...
    private function createCreateForm(Gender $entity)
    {
        $form = $this->createForm(new GenderType(), $entity, array(
            'action' => $this->generateUrl('wit_setting_gender_new'),
            'method' => 'POST',
            'attr' => array(
                'id'    => 'add-new-form',
            )
    ...
    public function newAction(Request $request)
    {
        ...
        if ($request->isMethod('POST')){
            ...
            if ($addform->isValid()) {
                ...
                return $this->redirect($this->generateUrl('wit_setting_gender_index'));
            }
    ...
    {
        $form = $this->createForm(new GenderType(), $entity, array(
            'action' => $this->generateUrl('wit_setting_gender_update', array('id' => $entity->getId())),
            'method' => 'PUT',
            'attr' => array(
                'id' => 'update-form',
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        $entity = $em->getRepository('ModelBundle:Gender')->find($id);
