...
use Wit\ModelBundle\Entity\Major;
use Wit\SettingBundle\Form\MajorType;
...
class MajorController extends Controller
{
    ...
    public function indexAction($id=null)
    {
        ...
        $entities = $em->getRepository('ModelBundle:Major')->findBy(array(), array('id' => 'DESC'));

        //form to create
        $addform = $this->createCreateForm(new Major());

        //if user wants to edit a record
        if($id){
            $editEntity = $em->getRepository('ModelBundle:Major')->find($id);
            if (!$editEntity) {
    ...
    private function createCreateForm(Major $entity)
    {
        $form = $this->createForm(new MajorType(), $entity, array(
            'action' => $this->generateUrl('wit_setting_major_new'),
            'method' => 'POST',
            'attr' => array(
                'id'    => 'add-new-form',
            )
    ...
    public function newAction(Request $request)
    {
        ...
        if ($request->isMethod('POST')){
            ...
            if ($addform->isValid()) {
                ...
                return $this->redirect($this->generateUrl('wit_setting_major_index'));
            }
    ...
    {
        $form = $this->createForm(new MajorType(), $entity, array(
            'action' => $this->generateUrl('wit_setting_major_update', array('id' => $entity->getId())),
            'method' => 'PUT',
            'attr' => array(
                'id' => 'update-form',
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        $entity = $em->getRepository('ModelBundle:Major')->find($id);
